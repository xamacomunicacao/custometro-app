...
export default function QuemSomosPage() {
  return (
    <main className="w-full min-h-screen bg-background font-sans overflow-x-hidden">
      ...
      <section className="max-w-2xl mx-auto px-6 md:px-0 py-20">

        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          <h2 className="text-[2.2rem] font-black text-[#1C2331] mb-3 tracking-tight">Quem somos?</h2>
          <p className="text-gray-600 text-[14px] leading-relaxed mb-10">
            Somos o <span className="text-[#FF0055] font-semibold">Custômetro</span>, uma plataforma dedicada a facilitar a acessibilidade e
            compreensão do uso da Cota para o Exercício da Atividade Parlamentar (CEAP) pelos
            parlamentares do Amazonas. Nosso propósito é oferecer transparência às informações
            públicas, transformando dados complexos em um formato claro e intuitivo, para que todos
            possam acompanhar e fiscalizar como os recursos públicos são utilizados.
          </p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
          <h2 className="text-[2.2rem] font-black text-[#1C2331] mb-3 tracking-tight">Por que existimos?</h2>
          <p className="text-gray-600 text-[14px] leading-relaxed mb-10">
            Sabemos que <span className="text-[#FF0055] font-semibold">informações sobre a gestão pública</span> muitas vezes são complicadas ou
            difíceis de acessar. Por isso, criamos um espaço onde esses dados são organizados e
            apresentados de forma transparente e intuitiva, tornando a fiscalização dos recursos
            públicos mais acessível a todos.
          </p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.15 }}>
          <h2 className="text-[2.2rem] font-black text-[#1C2331] mb-3 tracking-tight">Missão</h2>
          <p className="text-gray-600 text-[14px] leading-relaxed mb-10">
            <span className="text-[#FF0055] font-semibold">Empoderar o cidadão</span> por meio de informações públicas de forma acessível e organizada
            sobre como os parlamentares do Amazonas usam a Cota de Exercício de Atividade
            Parlamentar (CEAP). Assim, fortalecemos a transparência e a participação social.
          </p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }}>
          <h2 className="text-[2.2rem] font-black text-[#1C2331] mb-3 tracking-tight">Visão</h2>
          <p className="text-gray-600 text-[14px] leading-relaxed mb-10">
            <span className="text-[#FF0055] font-semibold">Empoderar o cidadão</span> por meio de informações públicas de forma acessível e organizada
            sobre como os parlamentares do Amazonas usam a Cota de Exercício de Atividade
            Parlamentar (CEAP). Assim, fortalecemos a transparência e a participação social.
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.25 }}>
          <h2 className="text-[2.2rem] font-black text-[#1C2331] mb-3 tracking-tight">Valores</h2>
          <p className="text-gray-600 text-[14px] leading-relaxed mb-12">
            Credibilidade, transparência, acessibilidade, cidadania, publicidade.
          </p>
        </motion.div>

        {/* Ícone dinheiro voando */}
        <motion.div
          animate={{ y: [0, -8, 0], x: [0, 4, 0], rotate: [0, -2, 2, -2, 0] }}
          transition={{ repeat: Infinity, duration: 6, ease: "easeInOut" }}
          className="flex justify-start ml-6"
        >
          <Image src="/ícone dinheiro voando.png" alt="Dinheiro voando" width={180} height={180} className="object-contain drop-shadow-md" />
        </motion.div>
      </section>
      ...
      <footer className="w-full bg-[#2A2F3B] text-white py-10 px-6 md:px-16 relative overflow-hidden">
        <div className="absolute top-6 right-8 opacity-10 pointer-events-none hidden md:block">
          <svg width="80" height="120" viewBox="0 0 80 120">
            <pattern id="footer-dots-qs" x="0" y="0" width="14" height="14" patternUnits="userSpaceOnUse">
              <circle fill="#ffffff" cx="2" cy="2" r="2" />
            </pattern>
            <rect x="0" y="0" width="80" height="120" fill="url(#footer-dots-qs)" />
          </svg>
        </div>
