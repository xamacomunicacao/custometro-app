...
function AnimatedCurrency({ value }: { value: number }) {
  ...
  return <motion.span ref={ref} className="text-[#FF0055]">{rounded}</motion.span>;
}

export default function DetalhesDespesas({ politico, despesas }: { politico: Politico, despesas: Despesa[] }) {
  // Calculando o total
  ...
  let backHref = "/senado";
  if (politico.cargo === "Deputado Federal") backHref = "/deputados-federais";
  if (politico.cargo === "Deputado Estadual") backHref = "/deputados-estaduais";
  if (politico.cargo === "Vereador") backHref = "/camara-municipal";
    ...
    <main className="w-full min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      <section className="relative z-20 max-w-5xl mx-auto px-6 md:px-12 py-10 flex flex-col items-center">
        ...
        <Link href={backHref} className="self-start">
          <motion.div 
            className="inline-flex items-center gap-2 text-[#FF0055] font-bold text-[15px] mb-8 cursor-pointer uppercase tracking-wide hover:opacity-80 transition-opacity"
            initial="initial"
            whileHover="hover"
          >
